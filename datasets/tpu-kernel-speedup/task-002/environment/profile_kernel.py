#!/usr/bin/env python3
"""TPU Hardware Profiler & Feedback Tool for RMSNorm Agent Refinement Loop."""
import ast
import hashlib
...
def main():
    ...
    if not hasattr(opt_mod, "run_kernel"):
        print(f"[ERROR] optimized_kernel.py must define a function 'run_kernel(X, W, eps)'.")
        sys.exit(1)
    ...
    print("================================================================================")
    print(f"       TPU RMSNorm Profiler Feedback [{backend_label}]")
    print("================================================================================")
    ...
    for b, s, d in test_shapes:
        ...
        X_t = jax.random.normal(k1, (b, s, d), dtype=jnp.float32)
        W_t = jax.random.normal(k2, (d,), dtype=jnp.float32)

        try:
            ref_out = base_mod.run_kernel(X_t, W_t).block_until_ready()
            opt_out = opt_mod.run_kernel(X_t, W_t).block_until_ready()
        except Exception as e:
            print(f"[VERIFICATION FAILED] Execution error for shape ({b}, {s}, {d}): {e}")
            sys.exit(1)

        # Check for NaN / Inf
        if bool(jnp.isnan(opt_out).any()) or bool(jnp.isinf(opt_out).any()):
            print(f"[VERIFICATION FAILED] Output contains NaN or Inf values for shape ({b}, {s}, {d})!")
            sys.exit(1)
        ...
        print(f"    - Shape (B={b}, S={s}, D={d}): Max Diff = {cur_diff:.6f} -> {'PASSED' if is_shape_accurate else 'FAILED'}")
        if not is_shape_accurate:
            print(f"[ERROR] Numerical accuracy exceeded error threshold for shape ({b}, {s}, {d}). Ensure kernel handles dynamic shapes.")
            sys.exit(1)

    is_accurate = max_diff_overall <= 1e-2

    # 2. Timing & Benchmark (Primary Shape B=4, S=1024, D=2048 with anti-memoization perturbation)
    B, S, D = 4, 1024, 2048
    ...
    if backend == "cpu":
        ...
        for i in range(n_iters):
            _ = base_mod.run_kernel(X + (i * 1e-7), W).block_until_ready()
        t1_b = time.perf_counter()
        base_lat_us = ((t1_b - t0_b) / n_iters) * 1e6
    else:
        base_lat_us = 120.0  # Baseline reference TPU latency

    t0 = time.perf_counter()
    for i in range(n_iters):
        _ = opt_mod.run_kernel(X + (i * 1e-7), W).block_until_ready()
    ...
    if speedup >= 1.5:
        ...
        print(f"[REFINEMENT NEEDED] Current speedup is {speedup:.2f}x. Optimize row tiling, memory fusion, and re-run.")
    print("================================================================================\n")
